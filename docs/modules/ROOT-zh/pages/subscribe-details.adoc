...
In a typical reactive application, users rarely explicitly call `subscribe()` themselves.
Instead, they declaratively define the processing pipeline as a `Flux` or `Mono` and
hand it to their library (like `reactor-netty`) or framework (like `Spring WebFlux`)
of choice to hook into a non-blocking execution engine (typically an event loop).
The user logic is composed of chaining the reactive operators.

There are times when you explicitly want to start the subscription and this section
aims to list the typical patterns.

=== Fire-and-forget

When you ignore the `Disposable` returned by `subscribe()`, you lose the ability to
cancel the execution. In most cases this is an anti-pattern,
but there are legitimate imperative boundaries (e.g. event listeners) where it is
unavoidable — in those cases, handle errors explicitly.
...
.Error handling at an imperative boundary: overload with error consumer
[source,java]
----
...
=== Dispatch and reference

For a more robust alternative, store the reference to the `Disposable` and perform
cleanup (`dispose()`) at a later stage.
...
The third option — providing the most control — is to extend `BaseSubscriber`. This is
appropriate when you need fine-grained backpressure control, custom cancellation logic,
or want to handle all lifecycle signals (`onSubscribe`, `onNext`, `onError`, `onComplete`,
`onCancel`)
in a single coherent class.

See <<an-alternative-to-lambdas-basesubscriber>> for a full example.