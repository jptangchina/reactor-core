...
xref:apdx-operatorChoice.adoc#which.errors[the relevant operator decision tree].
...
----
<2> The first lambda generates the resource. Here, we return our mock `Disposable`.
<3> The second lambda processes the resource, returning a `Flux<T>`.
<4> The third lambda is called when the `Flux` from <2> terminates or is cancelled, to
clean up resources.
<1> After subscription and execution of the sequence, the `isDisposed` atomic boolean
becomes `true`.
...
as shown in the xref:faq.adoc#faq.exponentialBackoff[FAQ].
...
xref:advancedFeatures/hooks.adoc#hooks-internal[hook].
...
managed to some extent by using customizable hooks. See xref:advancedFeatures/hooks.adoc#hooks-dropping[Dropping Hooks].
