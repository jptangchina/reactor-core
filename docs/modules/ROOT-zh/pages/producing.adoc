...
----
<2> Bridge to the `MyEventListener` API
<3> Each element in a chunk becomes an element in the `Flux`.
<4> The `processComplete` event is translated to `onComplete`.
<1> All of this is done asynchronously whenever the `myEventProcessor` executes.
...
----
<2> Poll for messages when requests are made.
<3> If messages are available immediately, push them to the sink.
<1> The remaining messages that arrive asynchronously later are also delivered.
