...
1. Can contain elements from different places in the original sequence.
1. Are never empty.
...
"air", "aim", "beta", "cat"`.
1. `concatMap` has concurrency of **1**. Therefore, the group with key `"a"` is the only
...
1. Next, `groupBy` requests additional 3 items (2 items are already buffered).
The buffer now contains `"beta", "cat", "ball", "apple", "bat"`. Out of these, `"apple"`
is consumed and the rest remain in the buffer.
1. Next, `groupBy` requests for additional 1 item (4 spaces are already occupied in
buffer). The buffered items are `"beta", "cat", "ball", "bat","dog"`.
1. Now, nothing in the buffer belongs to group `"a"`, hence no more consumption happens by
