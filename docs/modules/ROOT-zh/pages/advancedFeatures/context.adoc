...
StepVerifier.create(r)
            ...
            .verifyComplete();
----
<2> The chain of operators ends with a call to `contextWrite(Function)` that puts
`"World"` into the `Context` under a key of `"message"`.
<1> We `flatMap` on the source element, materializing the `ContextView` with `Mono.deferContextual()`
...
StepVerifier.create(r)
            ...
            .verifyComplete();
----
<4> This is the first write to happen.
<2> This is the second write to happen.
<1> The top context read sees second write.
<3> The `flatMap` concatenates the result from initial read with the value from the first write.
