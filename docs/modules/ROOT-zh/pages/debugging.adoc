...
1. A special suppressed exception is added to the original exception (or amended if already there).
1. A message is constructed for that special exception with several sections.
1. First section will trace back to the assembly site of the operator that fails.
1. Second section will attempt to display the chain(s) that are built from this operator and have seen the error propagate
1. Last section is the original stack trace
